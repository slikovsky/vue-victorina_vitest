import { _ as StringFromCodePoint, a as JSONStringify, c as ObjectAssign, f as ObjectFreeze, h as ObjectPrototype, i as JSONParse, l as ObjectCreate, m as ObjectKeys, o as MathMax, p as ObjectHasOwn, r as ArrayIsArray, s as MathMin, t as getErrorMessage, u as ObjectDefineProperty, v as SymbolIterator } from "./utils.js";
...
const INCLUDE_COMMENTS_SKIP_OPTIONS = {
	includeComments: !0,
	skip: 0
}, TokenProto = ObjectCreate(ObjectPrototype, { loc: {
	get() {
		return getNodeLoc(this);
	},
	enumerable: !0
} });
let tokens = null, comments = null, tokensAndComments = null, uint32$1 = null;
const TOKEN_TYPES = [
	"Identifier",
	"Keyword",
	"PrivateIdentifier",
...
function initTokens() {
	sourceText === null && initSourceText(), uint32$1 = buffer.uint32;
	let pos = uint32$1[536870901], len = uint32$1[536870902], endPos = pos + len * 16;
	for (tokens = []; pos < endPos;) tokens.push(deserializeToken(pos)), pos += 16;
	uint32$1 = null;
}
function deserializeToken(pos) {
	let pos32 = pos >> 2, start = uint32$1[pos32], end = uint32$1[pos32 + 1], value = sourceText.slice(start, end), kind = buffer[pos + 8], regex;
	if (kind <= 2) kind === 2 && (value = value.slice(1)), buffer[pos + 10] === 1 && (value = unescapeIdentifier(value));
	else if (kind === 8) {
...
function deserializeWith(buffer, sourceTextInput, sourceByteLen, getLocInput, deserialize) {
	return uint8 = buffer, uint32 = buffer.uint32, float64 = buffer.float64, sourceText$1 = sourceTextInput, sourceIsAscii = sourceText$1.length === sourceByteLen, getLoc = getLocInput, deserialize(uint32[536870900]);
}
...
const SOURCE_CODE = ObjectFreeze({
	...
	get tokensAndComments() {
		return tokensAndComments === null && (tokens === null && initTokens(), initTokensAndComments()), tokensAndComments;
	},
